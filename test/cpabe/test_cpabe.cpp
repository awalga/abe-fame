...
TEST (odn_cpabe, generate_pk) {
  ...
  auto pk = odn_cpabe::generate_pk(msk);
...
TEST (odn_cpabe, generate_sk) {
  ...
  auto sk = odn_cpabe::generate_sk(msk, set<string>{"A", "B", "C"});

  EXPECT_TRUE(sk.is_valid());
}

TEST (odn_cpabe, encrypt_decrypt) {
  using namespace ::odn::crypto;
  ...
  auto encrypt_session = odn_cpabe::encrypt_session(pk, policy);
  ...
  auto decrypt_session = odn_cpabe::decrypt_session(sk, encrypt_session.second);
