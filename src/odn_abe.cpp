...
*  Authority : MSK, PK
**********************************/
const cpabe_msk odn_abe::generate_authority(size_t klinear) {
  // msk param
...
const cpabe_pk odn_abe::generate_pk(const odn::crypto::cpabe_msk &msk) {
  // pk param
...
*                                       CPABE
*******************************************************************************************************/
cpabe_sk odn_abe::generate_sk(const odn::crypto::cpabe_msk &msk, const std::set<string> attributes) {
  const bilinear_pairing pairing(msk.pairing_param);
  ...
  for (size_t t = 0, l = pairing.klinear(); t != l; ++t) {
    ...
    for (size_t l = 0, i = pairing.klinear() + 1; l != i; ++l) {
      ...
      const string input_hash = string("01") + to_string(l + 1) + to_string(t + 1);// TODO encode string input as UTF_8
...
std::pair<sscipher_session, cpabe_cipher_text> odn_abe::encrypt_session(const odn::crypto::cpabe_pk &pk,
...
sscipher_session odn_abe::decrypt_session(const odn::crypto::cpabe_sk &sk,
  ...
  // create session with secret
  return sscipher::session(secret);
}

/******************************************************************************************************
*                                       KPABE Mode
*******************************************************************************************************/
kpabe_sk odn_abe::generate_sk(const odn::crypto::cpabe_msk &msk, const odn::crypto::binary_policy_tree &policy) {
  const bilinear_pairing pairing(msk.pairing_param);
  vector<field_zr> r_k;  // klinear + 1
  vector<field_zr> br_k; // klinear + 1
  // sk elements
   ...
   **********************************************************/
  // vector<group_g> pre_hash_table;
  vector<field_zr> sigma_j;
  // vector<group_g> g_sigma_j;
  // compute msp
  const msp lw_msp = lsss::lewko_water(policy);
  // pre compute hash
  for (size_t j = 0, num_cols = lw_msp.depth(); j != num_cols; ++j) {// TODO check depth < size
    /*for (size_t l = 0, k = msk.pairing_param.klinear(); l != k + 1; ++l) {

      for (size_t t = 0; t != k; ++t) {
        const string input_hash = string("0") + to_string(j + 1) + to_string(l + 1) + to_string(t + 1);

        pre_hash_table.push_back(group<group_g>::g_map(input_hash)
                                     ^ (br_k[l] / msk.a_k[t]));// Hash(concat(0, col, l, t)) ^ (br_k[t]/msk.a_k[t])
      }
    }*/
    // sigma_y <- Zp
    const field_zr sigma_j_ = group<field_zr>::zr().random();
    sigma_j.push_back(sigma_j_);
    /*for (size_t t = 0, k = msk.pairing_param.klinear(); t != k; ++t) {
      g_sigma_j.push_back(msk.g ^ (sigma_j_ / msk.a_k[t]));// g ^ sigma_j / a_k[t]
    }*/
  }
  // compute sky
  auto modCol = msk.pairing_param.klinear() * (msk.pairing_param.klinear() + 1);// t * l
  for (auto leaf : policy.attributes()) {
    auto attribute = leaf.first;
    auto rho_attr = lw_msp.get_share(leaf.first);// TODO != -1
    auto row_share = lw_msp[rho_attr];// TODO check size < depth

    const field_zr sigma_attr = group<field_zr>::zr().random();
    for (size_t t = 0, k = msk.pairing_param.klinear(); t != k; ++t) {
      // PI[l,j : H(attr, l, t) ^ (br_k[l]/a_k[t]) * H("0", j, l, t) ^ (br_k[l]/a_k[t]) ^ M(attr, j)]
      // * PI[j: g ^ (sigma_j/a_k[t]) ^  M(attr, j)]
      // * g ^ (sigma_attr/a_k[t])
      // * (g ^ d_k[t]) ^ M(attr, 0)
      group_g product_hash_attr = group<group_g>::g_zero();
      for (size_t l = 0; l != k + 1; ++l) {
        const string input_hash_attr = attribute + to_string(l + 1) + to_string(t + 1);
        // TODO crypto_generichash(sessionKey.key, KEY_SIZE, key, keylen, NULL, 0)
        const group_g hash_attr = group<group_g>::g_map(input_hash_attr);
        product_hash_attr += hash_attr ^ (br_k[l] / msk.a_k[t]);
        for (size_t j = 1, num_cols_share = row_share.size(); j != num_cols_share;
             ++j) { // TODO optimize loop
          // group_g hash_col(pre_hash_table[j * modCol + l * k + t]);
          const string input_hash_col = string("0") + to_string(j + 1) + to_string(l + 1) + to_string(t + 1);
          auto hash_col = group<group_g>::g_map(input_hash_col) ^(br_k[l] / msk.a_k[t]);
          product_hash_attr += (hash_col ^ group<field_zr>::zr_map(row_share[j]));
        }
      }
      for (size_t j = 1, num_cols_share = row_share.size(); j != num_cols_share; ++j) {
        //auto g_sigma_j = msk.g ^ ((sigma_j[j] / msk.a_k[t]) *  group<field_zr>::zr_map(row_share[j]));
        product_hash_attr += msk.g ^ ((sigma_j[j] / msk.a_k[t]) * group<field_zr>::zr_map(row_share[j]));
        //product_hash_attr +=
        //    (g_sigma_j[j * msk.pairing_param.klinear() + t]
        //       ^ group<field_zr>::zr_map(row_share[j]));// g ^ ((sigma_j / a_k[t]) ^  M(attr, col)))
      }
      product_hash_attr += (msk.g ^ (sigma_attr / msk.a_k[t]));
      product_hash_attr += (msk.g ^ (msk.d_k[t]) * group<field_zr>::zr_map(row_share[0]));
      // sk[y,t]
      sky.push_back(product_hash_attr);
    }
    // sk[y, k] = g^(-sigma_attr) * g ^ d_k[k] ^ M(attr, 1) * PI[j: g^(-sigma_j)^M(attr,j)]
    group_g sky_k = (msk.g ^ (-sigma_attr));
    sky_k += (msk.g ^ (msk.d_k[msk.pairing_param.klinear()] * group<field_zr>::zr_map(row_share[0])));
    for (size_t j = 1, num_cols_share = row_share.size(); j != num_cols_share; ++j) { // TODO optimize loop
      sky_k += (msk.g ^ ((-sigma_j[j]) * group<field_zr>::zr_map(row_share[j])));
    }
    sky.push_back(sky_k);
  }

  return kpabe_sk(msk.pairing_param, policy, std::move(sk0), std::move(sky));
}

std::pair<sscipher_session, kpabe_cipher_text> odn_abe::encrypt_session(const odn::crypto::cpabe_pk &pk,
                                                                        const std::set<string> attributes) {
// generate s <-Zp^k
  vector<field_zr> s_k;// klinear + 1
  // cipher components
  vector<group_h> ct0;
  map<string, vector<group_g>> cty;
  /**********************************************************
   * compute ct0[i] = h ^ s_k[i] i in [0..k+1(
   **********************************************************/
  field_zr sum_sk = group<field_zr>::zr_zero();
  ...
  **********************************************************/
  for (auto &attr : attributes) {
    for (size_t l = 0, k = pk.pairing_parm.klinear(); l != k + 1; ++l) {
      group_g hash_attr = group<group_g>::g_zero();
      for (size_t t = 0; t != k; ++t) {
        const string hash_input = attr + to_string(l + 1) + to_string(t + 1);
        hash_attr += group<group_g>::g_map(hash_input) ^ s_k[t];
      }
      cty[attr].push_back(hash_attr);
    }
  }
  /**********************************************************
  * compute ct = PI (i=[0..k[, T[i] ^ s[i]) * gt <- GT
  **********************************************************/
  ...
  const kpabe_cipher_text cipher_text(pk.pairing_parm, attributes, std::move(ct0), std::move(cty), std::move(ct));
  // create session with secret
  return {sscipher::session(secret), cipher_text};
}

sscipher_session odn_abe::decrypt_session(const odn::crypto::kpabe_sk &sk,
                                          const odn::crypto::kpabe_cipher_text &cipher_text) {
  // TODO check pairing params
  group_gt num(cipher_text.ct);
  group_gt den = group<group_gt>::gt_unity();

  auto min_authorization = sk.policy.root().prune(sk.policy, cipher_text.attributes);
  auto lw_msp = lsss::lewko_water(sk.policy);

  for (size_t l = 0, k = sk.pairing_param.klinear(); l != k + 1; ++l) {
    // PI(i=[0..col], cty[attr][l] ^ gamma[i])
    group_g ct_gamma = group<group_g>::g_zero();
    // sk1[l] * PI(i=[0..col], sk[attr[i]][l] ^ gamma[i])
    group_g sk_gamma = group<group_g>::g_zero();
    for (auto &attr : min_authorization) {
      auto sk_row = lw_msp.get_share(attr);// TODO check - 1
      ct_gamma += cipher_text.cty.find(attr)->second[l];
      sk_gamma += sk.sky[sk_row * (k + 1) + l];
    }
    // e(ct_gamma, sk0[l])
    num *= group<group_gt>::gt_unity()(ct_gamma, sk.sk0[l]);
    // e(sk_gamma, ct0[l])
